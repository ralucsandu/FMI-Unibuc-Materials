        ...
        for trans in l_transitions: #parcurg tranzitiile
            ...
            if trans[0] == current_state: #daca prima componenta este starea curenta
                if tape[i] == trans[1]: #daca s-a gasit o tranzitie din starea curenta cu simbolul la care s-a ajuns pe banda
                    ...
                    if trans[1]=='#' and trans[4]=='r': #daca tranzitia are pe prima pozitie # si se deplaseaza spre dreapta atunci se adauga la tape un spatiu
                        tape.append('_') #pentru ca spatiul de la finalul sirului va fi inlocuit cu un simbol (in partea dreapta a despartitorului '#' se creeaza suma numerelor)
                    if trans[3] != 'e': #daca se poate scrie ceva pe banda in locul unde arata capul
                        tape[i] = trans[3] #atunci caracterul care trebuie scris in acel spatiu ia locul caracterului existent
