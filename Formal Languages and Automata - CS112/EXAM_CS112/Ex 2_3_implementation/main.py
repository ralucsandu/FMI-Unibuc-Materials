#Membrii echipei: Cordun Diana Alexandra, Postolache Andreea Miruna, Sandu Raluca Ioana
#Important : s-a facut conventia ca stringurile de forma: ceva# sa fie acceptate
import sys
...
def validate(file_name):  #se valideaza fisierul de intrare
    ...
    if len(l_gamma)==0 or len(l_sigma) == 0 or len(l_states) == 0 or len(l_transitions) == 0: #daca cel putin o lista e goala se intoarce 2
        return 2
    ...
    for trans in l_transitions:
        ...
        if (tmp[0] not in l_states) or (tmp[1] not in l_gamma) or tmp[2] not in l_gamma or tmp[3] not in l_states or (tmp[4] not in l_gamma and tmp[4] != 'e')or (tmp[5] not in l_gamma and tmp[5] != 'e') or (tmp[6] != 'l' and tmp[6] !='r' and tmp[6]!='n')or (tmp[7] != 'l' and tmp[7] !='r' and tmp[7]!='n') :
            trans_ok = False
    ...
    while current_state != accept and current_state != reject: # cat timp nu a ajuns in stare de accept, respectiv reject
        for trans in l_transitions: #se parcurg tranzitiile
            ...
            if trans[0] == current_state: #daca prima componenta este starea curenta
                if tape[i] == trans[1] and tape[j]==trans[2]: #daca s-a gasit o tranzitie din starea curenta cu simbolul
                                                              #la care s-a ajuns cu capul stang su cu simbolul la care
                                                              # s-a ajuns cu capul drept
                    current_state = trans[3] #atunci starea curenta va deveni cea care apare pe pozitia a treia in lista de tranzitii
                    if trans[4] != 'e': #daca se poate scrie ceva pe banda in locul unde arata capul stang
                        tape[i] = trans[4] #atunci caracterul care trebuie scris in acel spatiu ia locul caracterului existent acolo
                    if trans[5] != 'e':#daca se poate scrie ceva pe banda in locul unde arata capul drept
                        tape[j] = trans[5]#atunci caracterul care trebuie scris in acel spatiu ia locul caracterului existent acolo
                    if trans[6] == 'l' and i != 0: # daca penultimul element este l atunci se decremeneteaza indicele pentru a se deplasa spre stanga capul stang
                        i = i - 1
                    if trans[7] == 'l' and j != 0: # daca ultimul element este l atunci se decremeneteaza indicele pentru a se deplasa spre stanga capul drept
                        j = j - 1
                    if trans[6]== 'r' and i<len(tape):# daca penultimul caracter este r si indicele unde se afla capul stang este mai mic decat tape-ul atunci se incrementeaza
                        i=i+1
                    if trans[7]== 'r' and j<len(tape):# daca ultimul caracter este r si indicele unde se afla capul drept este mai mic decat tape-ul atunci se incrementeaza
                        j=j+1
